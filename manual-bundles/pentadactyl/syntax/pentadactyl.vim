...
syn keyword pentadactylCommand loadplugins lpl gr[oup] ! run Clistk[eys] Clk
    ...
    \ ab[breviate] addo[ns] ao anc au[tocmd] ba[ck] background bg bd[elete]
    \ blistk[eys] blk bm[ap] bma[rk] bmarks bno[remap] b[uffer] buffers files ls
    \ tabs bunm[ap] ca[bbreviate] cal caret caretlistk[eys] caretlk caretm[ap]
    ...
    \ delm[arks] delqm[arks] dels[tyle] dia[log] di[ct] dic dictd did dictg dig
    \ dictq diq dictw diw dicty diy dictz diz dlc[lear] doautoa[ll] do[autocmd]
    \ downimageall downl[oads] dl ec[ho] echoe[rr] echom[sg] el[se] elsei[f]
    \ elif em[enu] en[dif] fi exe[cute] exit x exta[dd] extde[lete] extrm
    ...
    \ flashtoggle flt flushdns fo[rward] fw frameo[nly] gohttps ha[rdcopy]
    ...
    \ pref[erences] prefs proxy pw[d] qma[rk] qmarks q[uit] quita[ll] qa[ll]
    \ rea[dability] rea read[able] read redr[aw] reg[isters] reh[ash] re[load]
    \ reloada[ll] res[tart] runt[ime] sa[nitize] save-images si[mages] sav[eas]
    \ w[rite] sbcl[ose] scrip[tnames] search se[t] setg[lobal] setl[ocal]
    \ sideb[ar] sb[ar] sbop[en] sil[ent] siteSearch smartReader so[urce] spe[ak]
    ...
    \ tno[remap] toggleautoproxy aprxy toolbarh[ide] tbh[ide] toolbars[how]
...
syn keyword pentadactylOption activate act altwildmode awim autocomplete au
    \ caret_hint_key chk caret_hint_select_key chsk caret_hint_select_tail_key
    \ chstk caret_hint_tail_key chtk cdpath cd chinesehints chnh complete cpt
    \ cookieaccept ca cookielifetime cl cookies ck defsearch ds dict-engine dice
    \ dict-langpair dicl dict-show dico dictw-api dicwa downloadsort dlsort dls
    \ editor encoding enc eventignore ei extendedhinttags eht fbwhitelist fbw
    ...
    \ readability-style reas runtimepath rtp sanitizeitems si sanitizeshutdown
    \ ss sanitizetimespan sts scroll scr scrollsteps scs scrolltime sct shell sh
...
let s:toggleOptions = ["banghist", "bh", "dict-clipboard", "dicb",
    \ "dict-dblclick", "dicd", "dict-hasaudio", "dich", "dict-simple", "dics",
    \ "errorbells", "eb", "exrc", "ex", "flashblock", "fb", "fullscreen", "fs",
    \ "hlfind", "hlf", "incfind", "if", "insertmode", "im", "jsdebugger", "jsd",
    \ "more", "online", "private", "pornmode", "readability-local", "real",
